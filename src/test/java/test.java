...
public class test {
    ...
    private  ArrayList<ArrayList<Integer>> finPath(TreeNode root,int target,ArrayList<ArrayList<Integer>> resultList,ArrayList<Integer> pathList){
        ...
        if (pathList.get(0) > target ) {
            return resultList;
        }else if (pathList.get(0)==root.val&&root.left==null&&root.right==null){
    ...
    public static void main(String[] args){
        test test = new test();
        TreeNode node = test.createTree(new int[]{10,5 ,12,4,7});
        ArrayList<ArrayList<Integer>>  list = test.FindPath(node, 22);
